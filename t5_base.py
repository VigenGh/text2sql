...
try:
    finetuned_model = AutoModelForSeq2SeqLM.from_pretrained("finetuned_model_base_2_epoch")
...
if to_train:
    output_dir = f'./sql-training_base-{str(int(time.time()))}'
    ...
    finetuned_model.save_pretrained("finetuned_model_base_2_epoch")

questions = dataset['test'][:100]['question']
contexts = dataset['test'][:100]['schema']
human_baseline_answers = dataset['test'][:100]['sql']
